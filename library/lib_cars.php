<?php 
	class lib_cars extends Config {
		...
		public function showCars(){
			$data = [];
			$sql = $this->DB->query("SELECT a.carId AS 'carId', a.carModel AS 'carModel', a.plateNumber AS 'plateNumber', a.customerId AS 'customerId', b.fullName AS 'fullName' FROM ".$this->table['cars']." a INNER JOIN ".$this->table['customers']." b ON a.customerId = b.customerId WHERE a.status != '0'");
		...
		public function getCarRow($carId){
			$sql   = $this->DB->prepare("SELECT * FROM ".$this->table['cars']." WHERE carId = :carId ");
			$sql->execute([":carId" => $carId ]);
		...
		public function insertUpdateCar($data = []){
			...
			if(isset($data['carId']) && !empty($data['carId'])) { //Update Process
				...
				$sql   = $this->DB->prepare("SELECT * FROM ".$this->table['cars']." WHERE plateNumber = :plateNumber ");
